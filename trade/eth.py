...
class ContractCall:
    ...
    def __init__(self, config_path: str, router_abi_path: str, pair_abi_path: str, usdt_abi_path: str):
        ...
        """
        self.router_abi = read_abi(router_abi_path)
        self.pair_abi = read_abi(pair_abi_path)
        self.usdt_abi = read_abi(usdt_abi_path)
        ...
        self.router = self.w3.eth.contract(address=self.cfg.get("contract").get("router"), abi=self.router_abi)
        self.pair = self.w3.eth.contract(address=self.cfg.get("contract").get("pair"), abi=self.pair_abi)
        self.usdt = self.w3.eth.contract(address=self.cfg.get("contract").get("usdt"), abi=self.usdt_abi)
    ...
    def total_wallet_transfer_eth(self, new_path: str, sender: str, amount: str, private_key: str):
        """
        总钱包向子钱包转入原生代币
    ...
    def total_wallet_transfer_usdt(self, new_path: str, sender: str, amount: int, private_key: str):
        ...
        wallets = read_wallet_from_csv(new_path)
        for w in wallets:
            usdt = self.w3.eth.contract(address=self.cfg.get("contract").get("usdt"), abi=self.usdt_abi)
            option = {
                'chainId': self.w3.eth.chainId,
                'gas': 100000,
            ...
            ts = usdt.functions.transfer(w[0], amount).build_transaction(option)
